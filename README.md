...
## Vladimir exemplar status

The repacked Vladimir source has been changed so each embedded auxiliary-data
decode runs in a forked child of the declared compressor. This gives the cmix
model fresh process state without an additional `execve`; no second contestant
executable is launched. The stored `Entries/Vladimir/archive9` was generated by
that rebuilt compressor inside the pinned Ubuntu 22.04 judge environment. Its
full decompression and source-correspondence judgment must still complete before
the exemplar can be treated as an end-to-end passing result.
