...
IFS=$'\t' read -r dependency_build_image dependency_runtime_image \
  <<< "$dependency_images"
grep -qx 'qualification_os=ubuntu-22.04' \
  "$test_dir/dependencies/dependency-image.env"
grep -qx 'qualification_os_image=ubuntu:22.04@sha256:58b87898e82351c6cf9cf5b9f3c20257bb9e2dcf33af051e12ce532d7f94e3fe' \
  "$test_dir/dependencies/dependency-image.env"
[[ "$dependency_runtime_image" == \
  hutter-prize-entry-entry:ubuntu-22.04-*-runtime ]]
